...
5. [x] Create extension/ files:
   - [x] manifest.json: MV3 with permissions, host_permissions, content_scripts, background service_worker, commands (shortcuts).
   - [x] content.js: Inject shadow DOM #edubridge-root, mount React, message bridge to background, keyboard listeners.
   - [x] background.js: Handle tabCapture for audio, permission prompts, message router.
   - [x] content.css: Minimal global styles (if needed outside shadow).
6. [x] Create assets:
   - [x] public/isl/: Placeholder Lottie JSONs (neutral.json, phrase1.json, phrase2.json, phrase3.json).
   - [ ] public/icons/: Extension icons (16.png, 32.png, 48.png, 128.png - placeholders).
7. [x] Implement functionality:
   - [x] Audio input selector (mic/tab), real-time ASR pipeline (mock for demo).
   - [x] Modes (Speech/Sign-Lite/Auto), language select, TTS toggle (Web Speech fallback).
   - [x] ISL mapping from phrase-match, neutral loop fallback.
   - [x] Metrics (confidence, match %, latency), privacy banner.
   - [x] Buttons: Start/Stop, TTS, Braille export, Demo cycle, Settings sheet.
   - [x] Ribbon: aria-live, two lines, chips (lang/mode/offline), options menu (opacity/font/lock).
   - [x] Drag/resize/pin/close for overlays, auto-offset ribbon from Meet toolbar.
   - [x] Keyboard shortcuts relay (M/S/B, Ctrl+↑/↓, Esc).
   - [x] Error fallbacks: Toasts for API fails, demo mode offline.
8. [x] Create PWA files:
   - [x] public/manifest.webmanifest: For installable PWA.
   - [x] public/sw.js: Service worker for offline caching (assets, demo data).
9. [x] Create documentation:
   - [x] README.md: Setup, modes, shortcuts, permissions, adding phrases/assets, PWA build.
10. [ ] Build and test:
...
Progress: Steps 5-9 complete; building and testing next.
